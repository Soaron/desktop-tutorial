...
void osc_get_data(void)
{
  ...
  if(sync == 128) goto got_sync;
  {
    ...
    {
      /* wait for value > sync */
      __asm _osc_sync01:		MOVF	_PORTB, W		__endasm;		/* Copy(?) PORTB to W */
      __asm			SUBWF	_sync, W, B		__endasm; 					/* sync - PORTB */
      __asm			BNC	_osc_sync04		__endasm; 						/* if (c == 0) Goto _osc_sync04  ->  c == 0  Result is negative */
      __asm			MOVFF	_delayhi, _cnt3		__endasm; 				/* 2 c ; Set _cnt3 */ 
      __asm _osc_sync02:		MOVFF	_delay, _cnt2		__endasm; 	/* 2 c ; Set _cnt2 */
      __asm _osc_sync03:		DECFSZ	_cnt2, F, B 		__endasm; 	/* 1 c ; Decrease _cnt2 */
      __asm			BRA	_osc_sync03		__endasm; 						/* 2 c ; if (_cnt2 == 0) skip this line else goto _osc_sync03 */
      __asm			DECFSZ	_cnt3, F, B 		__endasm; 				/* 1 c ; Decrease _cnt3 */
      __asm			BRA	_osc_sync02		__endasm; 						/* 2 c ; if (_cnt3 == 0) skip else Goto _osc_sync02 */
      __asm			DECFSZ	_cnt, F, B 		__endasm; 					/* 1 c ; Decrease _cnt */
      __asm			BRA	_osc_sync01 		__endasm; 					/* 2 c ; if (_cnt == 0) skip else goto _osc_sync01*/
